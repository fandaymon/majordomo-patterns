...
class patterns extends module
{
    ...
    function admin(&$out)
    {
        ...
        if ($this->data_source=='categories') {
            if ($this->view_mode=='' || $this->view_mode=='search_categories') {
                ...
                $result=SQLSelect("SELECT * FROM pattern_categories ORDER BY ID");
                if ($result) {
                    $out['RESULT']=$result;
                }
            }
    ...
    function delete_categories($id) {
        $rec=SQLSelectOne("SELECT * FROM pattern_categories WHERE ID='$id'");
        // some action for related tables
        SQLExec("UPDATE patterns SET ID=0 WHERE CATEGORY_ID='".$rec['ID']."'");
        SQLExec("DELETE FROM pattern_categories WHERE ID='".$rec['ID']."'");
    }
